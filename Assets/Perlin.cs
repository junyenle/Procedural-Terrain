...
// Slightly modified to include Turbulence and Ridge functions
// Original author: Keijiro Takahashi, 2013, 2015
...
public static class Perlin
{
    ...
    public static float Turbulence(float x, float y, int octave)
    {
        Vector2 coord = new Vector2(x, y);
        var f = 0.0f;
        var w = 0.5f;
        for (var i = 0; i < octave; i++) {
            f += w * Mathf.Abs(Noise(coord));
            coord *= 2.0f;
            w *= 0.5f;
        }
        return f;
    ...
    // taken from https://thebookofshaders.com/13/
    public static float Ridge(float x, float y, int octave)
    {
        var o = 0.5f;
        Vector2 coord = new Vector2(x, y);
        var f = 0.0f;
